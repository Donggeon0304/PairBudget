const text = '카카오뱅크 입금 1원 명동건 → 입출금통장(1893)';
...
// 3. 은행명 및 쓸모없는 단어 제거
const tokens = ['카카오뱅크', '농협', '입금', '출금', '입출금통장', '입출금', '알림', '잔액'];
tokens.forEach(t => {
...
// 6. 숫자만 있는 단어 제거
const words = cleaned.split(' ').filter(w => !/^\d+$/.test(w));
const finalMerchant = words.join(' ').substring(0, 20);
